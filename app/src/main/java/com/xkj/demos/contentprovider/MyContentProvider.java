package com.xkj.demos.contentprovider;
...
import com.xkj.helpers.DBHelper;
...
public class MyContentProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        Log.i(TAG, " ArticleProvider.query " + uri);
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ...
        switch (uriMatcher.match(uri)) {
            case Articles.ITEM: {
                ...
                sqLiteQueryBuilder.setProjectionMap(articleProjectionMap);
                break;
            }
            case Articles.ITEM_ID: {
                String id = uri.getPathSegments().get(1);
                sqLiteQueryBuilder.setTables(DB_TABLE);
                sqLiteQueryBuilder.setProjectionMap(articleProjectionMap);
                sqLiteQueryBuilder.appendWhere(Articles.ID + "=" + id);
                break;
            }
            case Articles.ITEM_POS: {
        ...
        Cursor cursor = sqLiteQueryBuilder.query(db, projection, selection, selectionArgs, null, null, TextUtils.isEmpty(sortOrder) ? Articles.DEFAULT_SORT_ORDER : sortOrder, limit);
