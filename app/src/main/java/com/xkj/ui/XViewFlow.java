 ...
 */
public class XViewFlow extends AdapterView<Adapter> {
    ...
    public XViewFlow(Context context, AttributeSet attrs) {
        ...
        TypedArray styledAttrs = context.obtainStyledAttributes(attrs,
                R.styleable.XViewFlow);
        mSideBuffer = styledAttrs.getInt(R.styleable.XViewFlow_sidebuffer, 3);
        init(context);
    }
...
//        if (mIndicator != null) {
        ...
        int hPerceived = h + (mCurrentAdapterIndex - mCurrentBufferIndex)
                * getChildWidth();
        Log.d("ViewFlow", "hPerceived" + hPerceived + "h " + h + "v " + v);
//            mIndicator.onScrolled(hPerceived, v, oldh, oldv);
        ...
        if (mAdapter != null) {
            ...
            if (mContext instanceof Activity) {
                ...
                Log.d("ViewFlow", " width :" + width + "   height " + height);
                float positionOffset = ((float) hPerceived - (width * mCurrentAdapterIndex)) / width;
                Log.d("ViewFlow", "positionOffset" + positionOffset);
                int position=0;
                if (mViewChangeListener != null) {
//                    positionOffset = positionOffset < 0 ? 1 + positionOffset : positionOffset;
                    if(positionOffset<0){
                        positionOffset = 1+positionOffset;
                        position = mCurrentAdapterIndex-1;
                    }else{
                        position = mCurrentAdapterIndex;
                    }
                    mViewChangeListener.OnScrollChange(position, positionOffset);
                }
            }
        }
//        }
    ...
    public void setOnViewChangeListener(ViewChangeListener l) {
    ...
    public void setSelection(int position) {
        ...
        if (mViewChangeListener != null) {
            mViewChangeListener.OnPageChange(currentView, mCurrentAdapterIndex);
        }
    }
    ...
    private void postViewSwitched(int direction) {
        ...
        if (mViewChangeListener != null) {
            mViewChangeListener.OnPageChange(mLoadedViews.get(mCurrentBufferIndex), mCurrentAdapterIndex);
        }
        logBuffer();
    }
